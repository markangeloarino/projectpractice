...
class _EligibilityLicenseState extends State<EligibilityLicense> {
  ...
  final List<EligibilityRecord> _eligibilities = List.generate(
    3,
    (_) => EligibilityRecord(),
  );
  final List<LicenseRecord> _licenses = List.generate(
    3,
    (_) => LicenseRecord(),
  );
  ...
  Future<void> _loadEligibilityLicenseData() async {
    ...
    try {
      final String baseUrl = kIsWeb
          ...
          : 'http://10.0.2.2:3000';
      final url = Uri.parse(
        '$baseUrl/api/seekers/$seekerId/eligibilities-licenses',
      );
        ...
        if (mounted) {
          setState(() {
            ...
            for (int i = 0; i < eligList.length && i < 3; i++) {
              _eligibilities[i].nameCtrl.text =
                  eligList[i]['eligibility_name'] ?? '';
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Could not load eligibilities/licenses."),
    ...
    try {
      ...
      final response = await http
          .post(
            ...
            body: jsonEncode({"eligibilities": eligData, "licenses": licData}),
          )
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        // --- ELIGIBILITY SECTION ---
       BuildLabel(text:"ELIGIBILITY (Civil Service)"),
        ...
        // --- LICENSE SECTION ---
        BuildLabel(text: "PROFESSIONAL LICENSE (PRC)"),
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            flex: 3,
            child: GreyTextField(controller: 
              record.nameCtrl,
              hint: "Eligibility",
            ),
          ),
