    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          setState(() {
            ...
            final overseasLocs = locs
                .where((l) => l['is_overseas'] == 1)
                .toList();

            // Fill Local Inputs
            if (localLocs.isNotEmpty) {
              _local1Ctrl.text = localLocs[0]['location_name'] ?? '';
            }
            if (localLocs.length > 1) {
              _local2Ctrl.text = localLocs[1]['location_name'] ?? '';
            }
            if (localLocs.length > 2) {
              _local3Ctrl.text = localLocs[2]['location_name'] ?? '';
            }

            // Fill Overseas Inputs
            if (overseasLocs.isNotEmpty) {
              _overseas1Ctrl.text = overseasLocs[0]['location_name'] ?? '';
            }
            if (overseasLocs.length > 1) {
              _overseas2Ctrl.text = overseasLocs[1]['location_name'] ?? '';
            }
            if (overseasLocs.length > 2) {
              _overseas3Ctrl.text = overseasLocs[2]['location_name'] ?? '';
            }
          });
        }
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Could not load saved job preferences."),
  ...
  Future<void> _saveToDatabase() async {
    ...
    try {
      ...
      final Map<String, dynamic> formData = {
        ...
        "occupations": [
          {
            "job_title": _jobTitle1Ctrl.text.trim(),
            "company_name": _company1Ctrl.text.trim(),
          },
          {
            "job_title": _jobTitle2Ctrl.text.trim(),
            "company_name": _company2Ctrl.text.trim(),
          },
          {
            "job_title": _jobTitle3Ctrl.text.trim(),
            "company_name": _company3Ctrl.text.trim(),
          },
        ],
        "local_locations": [
          _local1Ctrl.text.trim(),
          _local2Ctrl.text.trim(),
          _local3Ctrl.text.trim(),
        ],
        "overseas_locations": [
          _overseas1Ctrl.text.trim(),
          _overseas2Ctrl.text.trim(),
          _overseas3Ctrl.text.trim(),
        ],
      };
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            BuildLabel(text: "PREFERRED OCCUPATION: "),
            BuildCheckbox(
              title: "Part-Time",
              value: _isPartTime,
              onChanged: (v) => setState(() => _isPartTime = v!),
            ),
        ...
        // ==========================================
        BuildLabel(text: "PREFERRED WORK LOCATION"),
        const Divider(color: Colors.black54, thickness: 1.5),

        // 1. Local (specify cities/municipalities
        BuildCheckbox(
          title: "Local (specify cities/municipalities)",
          value: _isLocal,
        ...
        BuildCheckbox(
          title: "Overseas (specify countries)",
          value: _isOverseas,
          onChanged: (v) => setState(() => _isOverseas = v!),
        ),
        const SizedBox(height: 15),
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(flex: 1, child: GreyTextField(controller: job)),
          const SizedBox(width: 15),
          Expanded(flex: 1, child: GreyTextField(controller: comp)),
        ],
      ),
    );
  }
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(flex: 1, child: GreyTextField(controller: loc)),
        ],
      ),
    );
  }
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(flex: 1, child: GreyTextField(controller: over)),
        ],
      ),
    );
  }
