  ...
  Future<void> _loadLanguageData() async {
    ...
    try {
      final String baseUrl = kIsWeb
          ...
          : 'http://10.0.2.2:3000';
      final url = Uri.parse(
        '$baseUrl/api/seekers/$seekerId/language-proficiency',
      );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Could not load language proficiencies."),
  ...
  Future<void> _saveToDatabase() async {
    ...
    try {
      ...
      final Map<String, dynamic> formData = {
        "eng_read": _engRead ? 1 : 0,
        "eng_write": _engWrite ? 1 : 0,
        "eng_speak": _engSpeak ? 1 : 0,
        "eng_understand": _engUnderstand ? 1 : 0,
        "fil_read": _filRead ? 1 : 0,
        "fil_write": _filWrite ? 1 : 0,
        "fil_speak": _filSpeak ? 1 : 0,
        "fil_understand": _filUnderstand ? 1 : 0,
        "man_read": _manRead ? 1 : 0,
        "man_write": _manWrite ? 1 : 0,
        "man_speak": _manSpeak ? 1 : 0,
        "man_understand": _manUnderstand ? 1 : 0,
        "other_language": _othersLangCtrl.text.trim(),
        "oth_read": _othRead ? 1 : 0,
        "oth_write": _othWrite ? 1 : 0,
        "oth_speak": _othSpeak ? 1 : 0,
        "oth_understand": _othUnderstand ? 1 : 0,
      };
      ...
      if (response.statusCode == 200) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text("Language Proficiencies saved!"),
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Padding(
                        ...
                        child: GreyTextField(controller: _othersLangCtrl),
                      ),
                    ),
                  ],
                ),
